...
2. 给Istio升级到1.8，看了下新特性感觉变化不太大，不过上次升级到1.7.2折腾到一半，只部署了istiod，plane没有切过去，今天刚好解决一下
   ...
   16G内存有点顶不住了，有空回家把内存挂咸鱼卖了换16x2 //TODO
...
3. 看到有NAT3优化到NAT1的文章，下次回家尝试优化，学校改为对称NAT之后只能和NAT1和NAT2打洞了，否则ICE要走TURN才能穿透，速度太慢 //TODO

### 2020/12/6

1. 学了一下hugo的文档，有时间给博客加个文章目录，不然日志太长了翻起来麻烦
2. 用dns劫持把DC1跑起来了，陈年todolist划掉一项，接入了node-red，这个东西感觉还挺好玩的，有空学一下文档之后加点功耗累计之类的功能 //TODO
...
2. 下午回家把大麦配好了，开了upnp之后测了一下成功full cone，可以STUN打洞
   ...
   回学校ping了一下延迟5ms，好评。有时间尝试一下wireguard on zerotier，虽然家里上行带宽只有20M-，但是考虑到延迟还是比小水管好用多了 // TODO
...
2. 把数据结构实验报告给搓了，近期还有离散，分析，对话三个ddl，明天必须结束，马原笔记周二抄一下 // TODO
...
1. 新年第一天当然是要快乐睡大觉（x
2. 说着搞复习还是睡了一整天，好困
3. 把MASS那边打工的活干完了，当个乙方还得自己改需求，挣口饭钱太难了（悲
4. vim好难适应啊麻了，写代码速度--，学一下trick

### 2021/1/2

1. dnspod突然开始要求实名，被迫搬家。试了几家之后发现he.net还不错，立刻迁移
2. 配了一上午vim，大物要完辽
3. **不能用竟是阿里NTP服务挂了，改完时间好起来了
4. 搞复习搞复习，再不复习人没了
...
3. 想起来js里import和require之类的一堆坑一直没捋明白 ，找了两篇文章
...
4. 把top的jq代码全部拿原生重写了，js真就是*山堆起来的语言啊麻了
...
2. 区块链打工，发现go的map其实是个引用——再加上编译器的逃逸分析，可以直接在函数里开map然后return出来
...
2. 学了一手mongo分片，开学之后试试在istio里把pulsar和mongo跑起来 // TODO
   ...
   丢家里的机子稳定性不错，回头装个linux上去，可以部署异地集群了 // TODO
...
1. 学了graphviz咋用，之后写篇博客梳理一个大型项目
   ...
   MinIO主要是业务代码，梳理的价值不太大，之后考虑下Pulsar
...
1. 开始着手组NAS，搞了一个rh2285hv2来玩，d3条子现在简直贵的离谱
2. 读deno学rust，rs不上手写点东西学不明白

### 2021/3/7

1. 路由器日常gg，ping和dns能通上不了网，排查半天发现是服务器gg
...
1. 捡的rh2285hv2服务器到了，freenas的BIOS模式大概有些问题，是很多遍还是起不来遂放弃
   ...
   还是当成母鸡用比较合适，proxmox ve成功跑起来

   结果装了一堆东西之后脑子一抽给rootfs缩了2G，当场变砖
...
1. 重建博客server，发现trustasia也需要认证了，换到`certbot`+`let's encrypt`
