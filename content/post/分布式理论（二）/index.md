...
$P_3$和$P_4$的读取子序列均为$\lbrace R(x)1 \rbrace}$,$\lbrace R(x)2,R(x)3 \rbrace}$，分别符合$P_1$及$P_2$的写入顺序。但由于$P_2$的写入操作前执行了$R(x)1$，导致$P_1$和$P_2$间产生了潜在的因果联系（比如购买商品时，首先读取库存数量，再对库存数量-1）。$P_4$观察到的整体顺序虽然分别符合$P_1$和$P_2$的操作序列，但是进程间操作的乱序可能造成因果倒置。显然，在这里添加`WFR`保证后${R(x)1}$将被强制排在$\{R(x)2,R(x)3\}$之前，即可唯一约束到$P_3$所见的顺序，这便得到了因果一致性模型。
